...
def derive_base_url(endpoint):
  parsed = urlparse(endpoint)
  path_parts = parsed.path.rsplit('/', 1)
  if len(path_parts) == 2 and path_parts[1]:
    base_path = path_parts[0] or '/'
  else:
...
def parse_args():
  parser = argparse.ArgumentParser(description='Generate an image through the CIII Responses API using OpenAI SDK.')
...
def main():
  ...
  base_url = derive_base_url(args.endpoint)
  client = openai.OpenAI(api_key=api_key, base_url=base_url)

  try:
    api_response = client.responses.create(**payload)
  except AttributeError:
    raise RuntimeError(
      'The installed openai SDK does not support the Responses API. '
      'Upgrade with: pip install --upgrade openai'
    )

  response_dict = api_response.model_dump()
  write_json_utf8(response_path, response_dict)

  if response_dict.get('error'):
    error_json = json.dumps(response_dict['error'], ensure_ascii=False, separators=(',', ':'))
    raise RuntimeError(f'API error: {error_json}')

  if response_dict.get('code') and response_dict.get('message'):
  ...
  image_bytes = base64.b64decode(extract_image_base64(response_dict))
